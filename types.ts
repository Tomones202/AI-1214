...
export interface ProductData {
  ...
  platform: string; // New: E-commerce Platform (e.g., 'tiktok', 'douyin', 'amazon')
  targetMarket: string; // Target Country Code (e.g., 'US', 'TH', 'CN')
  modelImages: string[]; // Custom model reference
  backgroundImages: string[]; // Custom background reference
  referenceVideo?: {
    data: string;
    mimeType: string;
  } | null; // Reference Video
}
...
export interface SceneDraft {
  id: string;
  visual: string; // 画面内容 (中文 - for User UI)
  visual_en: string; // Visual (English - for AI Generation)
  action: string; // 动作 (中文)
  action_en: string; // Action (English)
  camera: string; // 运镜 (中文)
  camera_en: string; // Camera (English)
  dialogue: string; // 对白 (Target Language: English, Thai, Chinese, etc.)
  dialogue_cn: string; // 对白 (中文翻译 - for reference)
  prompt: {
    imagePrompt: string; // Holds the V2 Veo JSON
  };
}
